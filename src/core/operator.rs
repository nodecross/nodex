...
impl UNiDDidOperator {
  pub fn new(_context: Option<UNiDDidResolverContext>) -> Self {
    ...
    let client_context: HttpClientContext = HttpClientContext { debug: debug };
...
pub mod tests {
  ...
  fn it_should_get_key_origin_and_value_nonempty() {
    ...
    assert_eq!(res.contains_key("origin"), true);
    assert_eq!(res.get("origin").is_some(), true);
    assert_eq!(res.get("origin").unwrap().is_empty(), false);
  }
  #[test]
  ...
  fn it_should_get_endpoint_nonempty() {
    ...
    let did: UNiDDidOperator = UNiDDidOperator::new(Some(resolver_context));
    assert_eq!(did.endpoint.is_empty(), false);
  }
  #[test]
